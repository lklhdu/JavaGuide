...
看完了javaGuide的三篇文章和尚学堂的`zookeeper`视频课，下面做一些总结，希望能看到这篇笔记时快速捡起一些`zookeeper`的知识
...
Zookeeper将所有数据存储在内存中，数据模型是一棵树（Znode Tree)，由斜杠（/）的进行分割的路径，就是一个Znode，例如/foo/path1。每个上都会保存自己的数据内容，同时还会保存一系列属性信息。（可以联系第四部分的数据模型）
...
![img](https://camo.githubusercontent.com/75e4e5b1b4ceeb1e297a5feea4d3c9af80ee70dd/68747470733a2f2f696d616765732e676974626f6f6b2e636e2f66343464383633302d316335352d313165392d623562372d616266306563306336363661)
...
连接服务后需要使用`zookeeper`提供的命令

![image-20200805101138762](http://java-guide.oss-cn-hangzhou.aliyuncs.com/typora/20200805101141-164760.png)
...
```shell
[zk: localhost:2181(CONNECTED) 2] create /node1 node1
Created /node1
```
...
```shell
[zk: localhost:2181(CONNECTED) 6] create /node1/node1.1 123
...
```shell
[zk: localhost:2181(CONNECTED) 7] set /node1 "set node1"
```
...
```shell
[zk: localhost:2181(CONNECTED) 10] ls2 /node1
[node1.1]
cZxid = 0x1c
ctime = Wed Aug 05 10:22:26 CST 2020
mZxid = 0x1e
mtime = Wed Aug 05 10:26:21 CST 2020
pZxid = 0x1d
cversion = 1
dataVersion = 1
aclVersion = 0
...
* [zookeeper相关概念总结](https://github.com/Snailclimb/JavaGuide/blob/master/docs/system-design/framework/ZooKeeper.md)
* [zookeeper原理简单入门](https://github.com/Snailclimb/JavaGuide/blob/master/docs/system-design/framework/ZooKeeper-plus.md)
* [zookeeper数据模型和常见命令](https://github.com/Snailclimb/JavaGuide/blob/master/docs/system-design/framework/ZooKeeper-plus.md)

2. [尚学堂视频课程](https://www.bilibili.com/video/BV1Up4y1X7eQ?t=538)
2. [关于视频课的一些笔记](https://blog.csdn.net/t1dmzks/category_7268582.html)
